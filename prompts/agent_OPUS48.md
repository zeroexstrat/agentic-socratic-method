# STANDING INSTRUCTION — agent OPUS48  (paste this as the session's task; it loops)

You are **OPUS48**, one of two agents converging the two source notebooks into one
merged notebook by taking turns through a shared folder. Your counterpart is
**SOL56**. You never call an API and you never talk to SOL56 directly — you
...
## Loop
1. **Wait for your turn** (this blocks — it's how you "listen" for SOL56's output):
   ```
   python coord/wait_turn.py --me OPUS48
   ```
...
2. **Do the phase named in the JSON.** Read the matching prompt and follow it exactly:
   ...
   | `audit`     | `prompts/turn_audit.md`       | `audits/audit_OPUS48_i{iter}.md` |
   | `metaaudit` | `prompts/turn_metaaudit.md`   | `metaaudits/metaaudit_OPUS48_i{iter}.md` |
   | `rework`    | `prompts/turn_rework.md`      | `rework/rework_OPUS48_i{iter}.md` + `product/product_i{iter+1}.ipynb` |
   ...
   - **Write every file atomically:** write to `NAME.tmp`, then `mv NAME.tmp NAME`
     (or use Python `os.replace`). This guarantees SOL56 never reads a half-written
   ...
   - On an **audit** turn: if you find NO new BLOCKER/MAJOR issue AND every blocking
     ...
     `prompts/signoff_template.md` → `signoff/SIGNOFF_OPUS48.md`. (This is the only
     way the run can end — but only if SOL56 independently signs the same version.)
...
3. **Hand off:**
   ```
   python coord/pass_turn.py --me OPUS48
   ```
   ...
   - Otherwise the baton has moved to SOL56. Go back to step 1.

## Hard boundaries
- Only ever write files whose name contains `OPUS48`, plus the shared `LEDGER.md`
  and (on your turns) `product/…`. **Never** write SOL56's audit/metaaudit/rework/
