...
The most useful evidence for the method was not the number of findings. It was the process’s ability to embarrass its own confidence.
...
Opus verified the counterexample, accepted the reopening, and retracted its earlier signoff. The retraction is one of the most important files in the archive even though the file itself no longer survives as the current signature. It demonstrates that a signoff was a falsifiable claim about a named artifact, not a ceremonial reward for having reached a late iteration.
...
This is what frozen did and did not mean. A confirmed repair could not be casually reverted. New evidence could still reopen it. The rule protected accumulated work from oscillation without turning previous judgment into law.
...
Here the naming must remain exact. Opus’s surviving signoff names `product_i7`. Sol’s current signoff names `product_i11`. The historical coordination stop still records convergence at `i7`. Therefore `i11` is the strongest final artifact in this repository and has a zero-waiver Sol signoff, but it does not satisfy the original two-model convergence predicate. Saying otherwise would erase the very provenance discipline the project was designed to test.
